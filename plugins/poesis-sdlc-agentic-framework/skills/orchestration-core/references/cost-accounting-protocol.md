# Cost Accounting Protocol (token cost of agentic development) — normative
...
> Companion of the [GitHub sync protocol](./github-sync-protocol.md): same house style, same
...
not new work). Measured on a real `ai-team-dev` dispatch: **33 turns**, `Σ inputTokens ≈ 3.99M` of which
...
|---|---|---|
| One Story's implementation/acceptance | that **Story** (`self`) | dev DRIVE, dev CRITIQUE, `ai-team-qa` acceptance, PO grooming of that Story |
| Feature-level work not tied to one Story | that **Feature** (`self`) | PM refinement (AC/WSJF), `SE: Architect` ADR, demo staging, Feature-level docs |
...
reads/writes). So the `S-NNN` / `F-NN` / `E-NN` is present in the dispatch's `userRequest` /
...
1. Select the dispatch logs whose prompt names the artifact id (`S-NNN` / `F-NN` / `E-NN`).
...
```text
E-05  cost.self   =  40k          (BO/EA shaping + PI Planning + I&A)
 ├─ F-11 cost.self =  18k         (PM refinement + ADR-003 + demo)
...
E-05.tokens_rolled = 40k + 72.7k + 21.4k          = 134.1k    (end-to-end, Epic E-05)

Point-per-point: S-102 cost 31.0k; F-11 own overhead 18k; E-05 own overhead 40k.
```
...
This is logged as a standing workflow pain point in
[`portfolio/_improvement-log.md`](../../../../portfolio/_improvement-log.md). The improvement (a future
