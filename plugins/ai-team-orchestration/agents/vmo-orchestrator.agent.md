...
2. [.github/skills/vmo-orchestration/SKILL.md](../skills/vmo-orchestration/SKILL.md) — your portfolio-layer procedure: Portfolio Init, ART / product registration, Strategic Themes, Epic intake + refinement + Gate 0, the Portfolio Kanban, Epic outcome acceptance, Strategic Portfolio Review, portfolio risk, and State Recovery.
...
- **★ Gate 0 (Epic)** facilitation; the Epic lifecycle through `portfolio-backlog -> implementing -> done`, including **Epic outcome acceptance** when the ART has demoed the Epic's last child Feature.
...
- **All program/ART-layer + iteration-layer execution** — dispatch **`@rte-orchestrator`** per ART once an Epic is in `portfolio-backlog` (Feature derivation/refinement, ★ Gate 2 ADR, PI Planning, System Demo / ★ Gate 4, PR merge / ★ Gate 3 via `@sm-orchestrator`, PI Inspect & Adapt). The rte-orchestrator in turn dispatches `@sm-orchestrator` for iterations.
...
- **No Gate skipping; no return before gate** — proceed autonomously until the ★ Gate 0 packet is ready.
...
- **Gate decision backlog** — portfolio-level decisions/assumptions are listed in the ★ Gate 0 packet with `accept` / `rework` / `defer`.
