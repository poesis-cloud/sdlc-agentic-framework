...
| Features | `docs/features/` | Features (F-N-<slug>.md) |
...
| Role | Agent | Owns |
|------|-------|------|
| RTE / Orchestrator | `@release-train-engineer` | Drives the SAFe flow, derives Stories, assigns pairs, runs acceptance, merges PRs after approval. Never writes production code. |
| Product Management | `@product-manager` | Features (`docs/features/F-N-<slug>.md`), Program Backlog, WSJF ranking |
| Enterprise Architecture | `@enterprise-architect` | Portfolio runway, NFR backbone, and enabler Epic shaping |
| System Architecture | `@system-architect` | ADRs (`docs/architecture/adr-N-*.md`, `status: proposed` until human approves) |
| Generic Dev | default Copilot agent | Code for Stories that don't need a specialist (dispatched by RTE) |
| Security | `@security-expert` | Auth, crypto, secrets, threat-model-driven code |
| Platform / CI | `@operator` | CI/CD, Helm, deployment, env config |
| Docs | `@tech-writer` | Story-attached docs, ADR prose, release notes |
| UX | `@ux-designer` | UX journeys, Figma specs |
| Accessibility / inclusivity | `@ux-designer` | Accessibility enablers, inclusivity review |
...
When the RTE chat overflows or restarts, the human says **`@release-train-engineer — recover state.`** RTE then:
...
**For infrastructure:** label `infra`. Routed to a pair with `@operator` as Driver or Navigator.
