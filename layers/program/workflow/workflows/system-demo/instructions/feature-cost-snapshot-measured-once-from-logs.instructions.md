---
description: 'the release-train-engineer writes the Feature `cost:` block exactly once at the ★ Demo Gate accept (`in-progress → done`), self overhead measured from the ecosystem debug logs (matched by `F-N`) plus committed child-Story rollups — never fabricated, immutable once committed'
---

# Invariant: feature_cost_snapshot_measured_once_from_logs

At the Feature's ★ Demo Gate accept (`in-progress → done`) the **release-train-engineer** commits the `cost:` block **exactly once**: the Feature's own overhead dispatches **measured** net-of-cache from the debug logs (matched by `F-N`), plus the sum of its already-committed child-Story `tokens_rolled`. Self overhead is **never fabricated as if measured**; child rollups are read from immutable committed snapshots, **not recomputed**; the Feature block is **immutable** once `committed` is set. The companion CEL postcondition `feature_cost_committed_once_on_accept` only checks that the block exists; this invariant governs its **provenance + write-once** discipline. The full normative model is inlined below.
