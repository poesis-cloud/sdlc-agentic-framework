...
- **`@value-management-officier`** — portfolio layer (Strategic Themes, Epics, the Epic Gate, Portfolio Kanban, ART registration, portfolio risk + Epic cost). See [value-management-officier skill](portfolio/actors/value-management-officier/value-management-officier.skill.md).
- **`@release-train-engineer`** — program / ART layer (Features, PI, ADR / Demo gates, ART health). See [release-train-engineer skill](program/actors/release-train-engineer/release-train-engineer.skill.md).
- **`@scrum-master`** — team / iteration layer (Stories, sprints, pair execution, PR gate prep). See [scrum-master skill](team/actors/scrum-master/scrum-master.skill.md).

It carries everything **common** to all three. Layer-specific flow lives in the three per-layer skills. When a per-layer skill diverges from this base on a shared mechanic, **this base wins**. The dispatch chain is `@value-management-officier -> @release-train-engineer -> @scrum-master`.
...
**The two producers (defined per tier, not here).** *Reactive* items come from the **Challenge-return loop** (above). *Proactive* items come from the **Discovery turn** that opens each **CE intake practice** — tier-scoped and defined *there*: portfolio CE = `epic-lean-business-case`, ART CE = `feature-backlog-refinement`, iteration CE = `story-backlog-refinement`. orchestrator owns only this ledger, its routing, and the gate fold; the per-tier Discovery lenses live with their practice.
...
**Canonical terminology (mandatory).** The single source for the orchestration vocabulary is [canonical-glossary.md](canonical-glossary.md): *orchestration* (ranks: *root orchestration* = vmo/rte/sm, *suborchestration* = the facilitated bodies below), *workflow* (`workflow.yaml` declaring *structurant steps*, each a flat `conditions` list of `{kind, type, expression, value}` records), *journal* (one per-run log, [journal.schema.json](../harness/schemas/journal.schema.json) — `portfolio/logs/<run>.jsonl`, one entry per harness command, discriminated by `command`), *artifact* (JSON source of truth, Markdown rendered last; schema mirrors the template's section tree), and *harness* (the deterministic CLI driver — see [harness README](../def/harness/README.md): `orchestrate`, `check-step`, `check-artifact`, `hook`). "Ceremony"/"practice" survive only as the `kind:` field; the canonical noun is **suborchestration**.

**Suborchestration nesting.** Every orchestration uses the step-based `workflow.yaml`, and every suborchestration now lives under `<root-orchestration>/workflows/<name>/`, loaded by **explicit path** (they leave VS Code auto-discovery; the root orchestrations + author roles stay flat at the framework root `<framework-root>/<name>/`).
...
- **Step 0 first:** if `portfolio/portfolio.yaml > products[]` does not list the product in scope, run ART / Product Init (see value-management-officier skill — the VMO owns the registry) before any other step.
...
1. **Hard block (default):** stop the handler, record a workflow pain point in `portfolio/_improvement-log.md`, and tell the Central Supervisor that the current environment cannot execute the required subagent dispatch.
...
from those logs **once**, at the artifact's terminal status, per the [cost-accounting protocol](../instructions/cost-accounting-protocol.md):
...
owner → processing instance → gate), not a procedure to replay by hand: they tell you *who* owns a column
and *which* transitions carry a ★ gate, so you can read a dispatch/halt and stage the right gate packet.
...
- **Token cost is fetched once from the ecosystem logs** at each artifact's terminal status (Story `awaiting-pr` / Feature `done` / Epic `done`) and rolled up Story -> Feature -> Epic per the [cost-accounting protocol](../instructions/cost-accounting-protocol.md) (`cost:` frontmatter blocks; **no intermediary ledger**); every figure is flagged `measured` or `estimated`, never fabricated.
...
- **Authoring vs policing.** The hat-wearing **author agents own the backlog artifacts**: `@business-owner` authors Epics + Strategic Themes, `@product-manager` authors Features, `@product-owner` authors Stories, `@enterprise-architect` authors the EA runway and enabler Epics, and `@system-architect` authors ADRs plus enabler Features and Stories. The **orchestrators never author or own these** — they *police* their layer (control I/O artifacts, enforce template + SAFe conformance, own the flow / gates / kanban / templates). The **Central Supervisor approves** at the gates. One Feature per product, each linked to its shared Epic.
...
- **GitHub Projects boards mirror the kanbans** (one Project per product); reconcile via
  ...
  (Portfolio Epics board; `@value-management-officier`), **Feature → ★ Architecture Gate** (product Program board;
  `@release-train-engineer`), **Story → ★ PR Gate** (product Team board; `@scrum-master`). No item
...
- **Flow, not content — route every substantive concern to its hat; never reason *for* a persona.** When the Central Supervisor (or any input) raises a *substantive* concern — architecture, repo/topology, design, scope, requirements, tradeoffs, risk acceptance, a proposed solution — the orchestrator MUST NOT answer it with its own analysis, recommended design, option-set, or decision-fork. Doing so pre-empts the owning hat, biases the gate, and breaks one-author-per-artifact. Instead **route it into the workflow**: capture it as the *input* to the owning hat's ceremony/practice, dispatch that hat (PM → Feature, SA/EA → ADR/runway, PO → Story, QA/Security/DevOps/UX → their lens), and **police** the returned artifact. The orchestrator authors only *flow/meta* artifacts (kanban, pi-objectives, risks, gate packets, the improvement ledger) and reasons only about **flow** (which handler fires, which hat owns it, which gate is next, WIP/routing). "I understand the concern" is never license to solve it.
...
The orchestration workflow improves itself through a closed loop recorded in **one** append-only file, `portfolio/_improvement-log.md` (template: [workflow-improvement-ledger.artifact.schema.json](../harness/schemas/artifact/workflow-improvement-ledger.artifact.schema.json) + [workflow-improvement-ledger.artifact-template.md](artifacts/workflow-improvement-ledger.artifact-template.md)). Each entry carries both an **input** block (pain point) and an **output** block (improvement); keep the two **distinct within the entry** — a pain point is an *input*, not yet an improvement.

1. **Capture (continuous).** While orchestrating, when you hit **friction in the workflow itself** — a clunky/ambiguous step, a missing or wrong template field, a kanban/transition rule that fought you, a routing/dispatch gap, a redundant or contradictory instruction, an agent missing a tool/capability — append a **pain point** (input block only, `status: open`) to `portfolio/_improvement-log.md` immediately. Capture only: what happened, where it surfaced (portfolio/product/sprint/PI), and the *symptom*. Do **not** solve it inline. `@value-management-officier` captures portfolio-level friction; `@release-train-engineer` captures program/ART-level friction; `@scrum-master` captures iteration-level friction.
2. **Review (at retro / I&A / portfolio review).** Open pain points are pulled in as **input**: the **Retro** (scrum-master, sprint-scope), **Inspect & Adapt** (release-train-engineer, PI-scope), and **Strategic Portfolio Review** (value-management-officier, portfolio-scope) read the open entries, root-cause, and triage each into an *improvement* or a deliberate `wont-fix`.
...
- surfaced-by: value-management-officier | release-train-engineer | scrum-master | Central Supervisor | <subagent>
...
Artifact schemas and templates are distributed to the producing skills under [artifacts/](artifacts). Refuse to author an artifact without consulting its schema/template pair.
...
The `*.artifact.schema.json` file is the deterministic source for artifact identity, path matching, required frontmatter shape, derived heading/body checks, and render-only classification. The Python harness parses each Markdown artifact into normalized JSON data (frontmatter plus derived fields `__path`, `__kind`, `__body`, and `__sections` — an object keyed by each `##` heading whose value is that section's Markdown body) and validates that data with the standard `jsonschema` library. The `*.artifact-template.md` file is the authoring/rendering aid and may show syntax examples, but it must not be the only home of a deterministic validation rule. When a rule can be executed reliably, move it into the artifact schema or a handler `workflow.yaml` token and lighten the Markdown prose.
...
**Ban on non-standard authoritative artifacts.** Orchestrators and author agents must not introduce, preserve, or rely on ad hoc artifact classes such as standalone "contract surface" documents, topology notes, or other unlabeled design sidecars as authoritative homes. If such a document exists historically, reduce it to a transient migration aid at most, then migrate or delete its decisive content into the standard artifact set. A gate packet that still depends on a non-catalog authoritative artifact is incomplete.

**Artifact ownership (by layer).** Each orchestrator **owns and maintains** its tier's artifact schemas and artifact templates, and enforces conformance; cross-cutting ones stay in orchestrator. Authors always use the current owned version.
...
- **`release-train-engineer` owns:** feature, adr, architecture-decision-inventory, vision, roadmap, nfr-register, runway-register, pi-objectives, risks, inspect-adapt, kanban-program.
- **`scrum-master` owns:** sprint-plan, story, qa-signoff, daily, retro, progress, gate-decisions, kanban-team.
- **`orchestrator` owns (cross-cutting artifacts):** workflow-improvement-ledger, project-brief (under `orchestrator/artifacts/`), plus the canonical glossary and brainstorm-format (under `orchestrator/`). Framework-wide, host-agnostic concerns live in dedicated top-level homes the orchestrator references but no single layer owns: the techno-agnostic protocols cost-accounting / logging / anti-patterns (under `instructions/`) and the host-binding board spec / sync protocol / sync config (under `sync/github/`, swappable for another host adapter).
...
| Workflow improvement ledger (singleton) | `portfolio/_improvement-log.md` | [workflow-improvement-ledger.artifact.schema.json](../harness/schemas/artifact/workflow-improvement-ledger.artifact.schema.json) + [workflow-improvement-ledger.artifact-template.md](artifacts/workflow-improvement-ledger.artifact-template.md) |
| PI objectives | `pi-M/pi-objectives.md` | [pi-objectives.artifact.schema.json](../harness/schemas/artifact/pi-objectives.artifact.schema.json) + [pi-objectives.artifact-template.md](program/actors/release-train-engineer/artifacts/pi-objectives.artifact-template.md) |
| PI risks | `pi-M/risks.md` | [risks.artifact.schema.json](../harness/schemas/artifact/risks.artifact.schema.json) + [risks.artifact-template.md](program/actors/release-train-engineer/artifacts/risks.artifact-template.md) |
| Inspect & Adapt | `pi-M/inspect-adapt.md` | [inspect-adapt.artifact.schema.json](../harness/schemas/artifact/inspect-adapt.artifact.schema.json) + [inspect-adapt.artifact-template.md](program/actors/release-train-engineer/artifacts/inspect-adapt.artifact-template.md) |
| Program Kanban (rendered) | `kanban/program.md` | [kanban-program.artifact.schema.json](../harness/schemas/artifact/kanban-program.artifact.schema.json) + [kanban-program.artifact-template.md](program/actors/release-train-engineer/artifacts/kanban-program.artifact-template.md) |
...
| Cost accounting protocol (normative) | — (`cost:` blocks; sourced from ecosystem debug logs) | [cost-accounting-protocol.md](../instructions/cost-accounting-protocol.md) |
| Project brief (per repo) | `<repo>/PROJECT_BRIEF.md` | [project-brief.artifact.schema.json](../harness/schemas/artifact/project-brief.artifact.schema.json) + [project-brief.artifact-template.md](artifacts/project-brief.artifact-template.md) |

## Anti-patterns

See [anti-patterns.md](../instructions/anti-patterns.md).
